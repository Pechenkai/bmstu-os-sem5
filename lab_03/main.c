...
void daemonize(const char* name) 
{
    ...
    if (getrlimit(RLIMIT_NOFILE, &r_limit) == -1)
...
void reread(void) 
{
    ...
    if ((fd = fopen("/etc/daemon.conf", "r")) == NULL)
        syslog(LOG_ERR, "Ошибка открытия файла etc/daemon.conf");
    else
    {

        fscanf(fd, "%s %d", name_buf, &uid);
        fclose(fd);
    ...
    for (;;) 
    {
        ...
        switch (signo) 
        {
            ...
            case SIGTERM:
                ...
                exit(0);
            default:
                syslog(LOG_INFO, "Полученный сигнал: %d\n", signo);
                break;
        }
    }
    return (0);
...
int main(int argc, char* argv[]) 
{
    ...
    FILE* file = fopen("/etc/daemon.conf", "w");
